...
while True:
	...
	if ids is not None:
		...
		# print("pitch yaw (0-360 range):", theta_x_360, theta_z_360)
		...
		if len(ids) >= 4:
			...
			# Print the probability grid
			count_above_threshold = np.sum(probability_grid >= 0.8)
			zero_threshold = np.sum(probability_grid<0.8)

			print("Probability grid size -- ", probability_grid.shape)
			print("Probability Grid with >=0.8---", count_above_threshold)
			print("Probability Grid with probability == 0.0---", zero_threshold)
			...
			# Thresholded Image
			plt.subplot(1, 2, 2)
			plt.imshow(probability_grid, cmap='gray')
			...
			# Update the original frame with the cropped result
    		#frame = frame[:height, :width]

			
			print("Corners = ", four_corners)
			bottom_right = tuple(map(int, corners[3][0][2]))
			bottom_left = tuple(map(int, corners[2][0][3]))
			top_right = tuple(map(int, corners[1][0][1]))
			top_left = tuple(map(int, corners[0][0][0]))
			cv2.line(frame, bottom_right, bottom_left, (0, 255, 0), 2)
			cv2.line(frame, bottom_right, top_right, (0, 255, 0), 2)
